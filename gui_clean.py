...
def main():
    ...
    else:        if loaded_translations is not None:
            if 'import_errors' in globals() and isinstance(globals()['import_errors'], list):
    ...
    if selected_page_key == "input":
        ...
        if all(m is not None for m in required_modules_for_admin_render) and callable(getattr(admin_panel_module, 'render_admin_panel', None)):
            ...
            st.warning(f"Admin-Panel oder dessen Abhängigkeiten ({', '.join(missing_modules_admin_list)}) nicht verfügbar.")
        ...
        if doc_output_module and database_module and product_db_module and callable(getattr(doc_output_module, 'render_pdf_ui', None)):
            ...
            st.warning("PDF-Ausgabemodul oder dessen Abhängigkeiten sind nicht verfügbar.")
        ...
        if ai_companion_module and callable(getattr(ai_companion_module, 'render_ai_companion', None)):
            ...
            except Exception as e_render_ai:
                st.error(f"Fehler beim AI-Begleiter: {e_render_ai}")
                st.text_area("Traceback AI-Begleiter:", traceback.format_exc(), height=200)
        else:
            st.warning("⚠️ AI-Begleiter Modul nicht verfügbar.")
            st.info("Das AI-Begleiter Modul (ai_companion.py) konnte nicht geladen werden.")
        ...
        if multi_offer_module and callable(getattr(multi_offer_module, 'render_multi_offer_generator', None)):
            ...
            except Exception as e_render_multi:
                st.error(f"Fehler beim Multi-Angebotsgenerator: {e_render_multi}")
                st.text_area("Traceback Multi-Angebote:", traceback.format_exc(), height=200)
        else:
            st.warning("⚠️ Multi-Angebote Modul nicht verfügbar.")
            st.info("Das Multi-Angebote Modul (multi_offer_generator.py) konnte nicht geladen werden.")


if __name__ == "__main__":
    try:
        # Alle bestehenden Module importieren
        ...
        # Berechnungsmodule initialisieren
        ...
        # Hauptanwendung starten        if database_module:
            main()
        else:
...
# Änderungshistorie
# 2025-06-16: Erweitert um AI-Begleiter und Multi-Angebotsgenerator
#             Alle bestehenden Funktionen beibehalten, neue Features integriert
#             Verbesserte Fehlerbehandlung für neue Module

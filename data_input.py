...
def render_data_input(texts: Dict[str, str]) -> None: 
    ...
    if 'customer_data_expanded_di' not in st.session_state:
        st.session_state.customer_data_expanded_di = True # Default zu expanded
    ...
    with st.expander(get_text_di(texts, "customer_data_header", "Kundendaten"), expanded=st.session_state.customer_data_expanded_di):
        ...
        if not (abs(current_lat) < 1e-9 and abs(current_lon) < 1e-9) :
            ...
            if st.session_state.get('satellite_image_url_di'): 
                ...
                try:
                    ...
                    inputs['project_details']['visualize_roof_in_pdf_satellite'] = False 
                    inputs['project_details']['satellite_image_base64_data'] = None
                    inputs['project_details']['satellite_image_for_pdf_url_source'] = None
        ...
        inputs['customer_data']['income_tax_rate_percent']=st.number_input(label=get_text_di(texts,"income_tax_rate_label","ESt.-Satz (%)"),min_value=0.0, max_value=100.0,value=float(inputs['customer_data'].get('income_tax_rate_percent',0.0) or 0.0),step=0.1,format="%.1f",key='income_tax_rate_percent_di_v6_exp_stable',help=get_text_di(texts,"income_tax_rate_help","Grenzsteuersatz für Wirtschaftlichkeitsberechnung (optional)"))
    ...
    if 'consumption_data_expanded_di' not in st.session_state: st.session_state.consumption_data_expanded_di = True
    ...
    if 'building_data_expanded_di' not in st.session_state: st.session_state.building_data_expanded_di = True
    with st.expander(get_text_di(texts,"building_data_header","Daten des Gebäudes"),expanded=st.session_state.building_data_expanded_di):
        ...
        with col_build1:
            ...
            # if build_year_val<1960:st.warning(get_text_di(texts,"build_year_warning_old","❗️ Zählerschrank/Hauselektrik prüfen.")) # Optional
            # elif build_year_val<2000:st.info(get_text_di(texts,"build_year_info_mid","ℹ️ Zählerschrank/Hauselektrik prüfen.")) # Optional
        ...
        with col_build3:
            ...
            # if inputs['project_details']['roof_covering_type']and inputs['project_details']['roof_covering_type']in['Schiefer','Bitumen','Eternit']:st.warning(get_text_di(texts,"roof_covering_warning","❗️ Höhere Montagekosten möglich.")) # Optional
    ...
    if 'tech_selection_expanded_di' not in st.session_state: st.session_state.tech_selection_expanded_di = True
    ...
    inputs['project_details']['include_additional_components']=st.checkbox(get_text_di(texts,"include_additional_components_label","Zusätzliche Komponenten einplanen"),value=inputs['project_details'].get('include_additional_components',False),key='include_additional_components_di_tech_main_cb_v6_exp_stable')
    if inputs['project_details']['include_additional_components']:
        def create_component_selector(component_label_key: str, component_list: List[str], project_details_key_name: str, project_details_key_id: str, widget_key_suffix:str):
            ...
            selected_name=st.selectbox(label_str,options=options,index=initial_idx,key=f"{project_details_key_name}_widget_key_di_add_{widget_key_suffix}_v6_exp_stable") 
            inputs['project_details'][project_details_key_name]=selected_name if selected_name!=please_select_text else None
            st.session_state[project_details_key_name] = inputs['project_details'][project_details_key_name] 
        ...
        create_component_selector("wallbox_model_label",WALLBOX_LIST_MODELS,"selected_wallbox_name","selected_wallbox_id", "wb_v6_exp_stable")
        create_component_selector("ems_model_label",EMS_LIST_MODELS,"selected_ems_name","selected_ems_id", "ems_v6_exp_stable")
        create_component_selector("optimizer_model_label",OPTIMIZER_LIST_MODELS,"selected_optimizer_name","selected_optimizer_id", "opti_v6_exp_stable")
        create_component_selector("carport_model_label",CARPORT_LIST_MODELS,"selected_carport_name","selected_carport_id", "cp_v6_exp_stable")

...
def render_co2_savings_value_switcher(analysis_results: Dict[str, Any], texts: Dict[str, str], viz_settings: Dict[str, Any]):
    ...
    try:
        ...
        # Rufe die neue 3D-CO₂-Visualisierung auf
    ...
    if years_effective <= 0: 
        st.warning(get_text(texts,"viz_data_invalid_years_co2_switcher_v2","Ungültige Simulationsdauer für CO2-Diagramm.")); 
    ...
    if not (isinstance(annual_prod_sim_raw,list) and len(annual_prod_sim_raw)==years_effective):
        st.warning(get_text(texts,"viz_data_missing_co2_prod_switcher_v2","CO2: Jährl. Produktionsdaten unvollständig.")); 
        analysis_results['co2_savings_value_switcher_chart_bytes']=None; 
        return
    ...
    # Verbesserter CO₂-Preis mit realistischen Annahmen
    co2_price_eur=55; co2_price_increase=0.08  # Erhöhte Preise und Steigerungsrate
    ...
    if any(math.isnan(v) or math.isinf(v) for v in wert_co2_eur_per_year):
        st.warning(get_text(texts,"viz_data_nan_inf_co2_switcher_v2","CO2: Ungültige Werte (NaN/Inf) in Diagrammdaten.")); 
        analysis_results['co2_savings_value_switcher_chart_bytes']=None; 
        return
    
    # Erweiterte 3D-Visualisierung mit mehr Details
    fig=go.Figure()
    selected_palette = viz_settings.get("default_color_palette","Greens"); 
    valid_continuous_scales = ['Greens','Blues','Viridis','Plotly3','Solar','Hot']
    marker_colorscale = selected_palette if selected_palette in valid_continuous_scales else "Greens"
    
    # Hauptspur: CO₂-Einsparung und monetärer Wert
    fig.add_trace(go.Scatter3d(
        ...
        mode='markers+lines',
        marker=dict(
            color=wert_co2_eur_per_year,
            colorscale=marker_colorscale,
            size=8,
            showscale=True,
            colorbar=dict(
                title_text='Monetärer Wert (€)',
                title_side='right'
            )
        ),
        line=dict(color='green', width=4),
        name='CO₂-Wert-Entwicklung',
        hovertemplate='<b>Jahr %{x}</b><br>' +
                      'CO₂-Einsparung: %{y:.2f} Tonnen<br>' +
                      'Monetärer Wert: %{z:.0f} €<extra></extra>'
    ))
    
    # Zusätzliche Referenzlinie für kumulierte Werte
    kumulierte_co2 = np.cumsum(co2_savings_tonnes_per_year)
    kumulierte_werte = np.cumsum(wert_co2_eur_per_year)
    ...
    fig.add_trace(go.Scatter3d(
        ...
        z=kumulierte_werte,
        mode='lines+markers',
        marker=dict(color='darkgreen', size=6, symbol='diamond'),
        line=dict(color='darkgreen', width=3, dash='dash'),
        name='Kumulierte Werte',
        hovertemplate='<b>Jahr %{x}</b><br>' +
                      'Kumulierte CO₂-Einsparung: %{y:.1f} Tonnen<br>' +
                      'Kumulierter Wert: %{z:.0f} €<extra></extra>'
    ))
    
    fig.update_layout(
        title=dict(
            text=get_text(texts,"viz_co2_savings_value_title_switcher","CO₂-Ersparnis und simulierter monetärer Wert über Zeit"),
            font=dict(size=16)
        ),
        scene=dict(
            xaxis_title='Jahr',
            yaxis_title='CO₂-Einsparung (Tonnen)',
            zaxis_title='Simulierter Wert (€)',
...
def render_selfuse_stack_switcher(analysis_results: Dict[str, Any], texts: Dict[str, str], viz_settings: Dict[str, Any]):
    ...
    for i,year_label in enumerate(jahre_sim_labels):
        ...
        fig.add_trace(go.Scatter3d(x=[year_label,year_label],y=[bar_width_y*1.5,bar_width_y*1.5],z=[0,einspeisung_sim_kwh[i]],mode='lines',line=dict(width=15,color=color_feedin),name='Einspeisung (kWh)' if i==0 else None,legendgroup='Einspeisung',showlegend=(i==0),hoverinfo="x+z"))
    fig.update_layout(title=get_text(texts,"viz_selfuse_stack_title_switcher","Jährlicher Eigenverbrauch vs. Einspeisung (Simuliert)"),scene=dict(xaxis_title='Simulationsjahr',yaxis_title='Kategorie',yaxis=dict(tickvals=[0,bar_width_y*1.5],ticktext=['Eigenverbrauch','Einspeisung'],range=[-0.5,bar_width_y*1.5+0.5]),zaxis_title='Energie (kWh)'),margin=dict(l=0,r=0,b=0,t=60),legend=dict(yanchor="top",y=0.99,xanchor="left",x=0.01))
...
def get_pricing_modifications_data():
    ...
    """
    # Hole die Live-Berechnungen aus dem Session State
    live_calc = st.session_state.get('live_pricing_calculations', {})
    
    # Hole die Slider-Werte
    ...
    descriptions = {
        ...
        'special_costs': st.session_state.get('pricing_modifications_descriptions_special_costs_text', ''),
...
def render_pricing_modifications_ui():
    ...
    # Live-Kosten-Vorschau mit korrekter Berechnung
    st.markdown("### Live-Kosten-Vorschau")
    
    # Hole Grundpreis aus calculations.py oder fallback
    ...
    # Hole die aktuellen Slider-Werte für Live-Berechnung
    discount_percent = st.session_state.get('pricing_modifications_discount_slider', 0.0)
    rebates_eur = st.session_state.get('pricing_modifications_rebates_slider', 0.0)
    surcharge_percent = st.session_state.get('pricing_modifications_surcharge_slider', 0.0)
    special_costs_eur = st.session_state.get('pricing_modifications_special_costs_slider', 0.0)
    ...
    # Aktualisiere Session State mit berechneten Werten für Verknüpfung mit anderen Berechnungen
...
# --- Haupt-Render-Funktion ---
    ...
    st.session_state['live_pricing_calculations'] = {
        ...
        'surcharge_amount': surcharge_amount,        'price_after_discounts': price_after_discounts
    }
...
def render_analysis(texts: Dict[str, str], results: Optional[Dict[str, Any]] = None) -> None:
    ...
    # INTEGRATION: Live-Preisberechnungen in calculation_results integrieren
    ...
    if 'calculation_results' in st.session_state and pricing_data['has_modifications']:
        ...
        # Aktualisiere die calculation_results mit den Live-Berechnungen
        if live_calc:
            ...
            # Aktualisiere auch total_investment_netto und _brutto mit den angepassten Werten
